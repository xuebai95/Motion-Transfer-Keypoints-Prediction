...
from Training_Prediction.FOMM.Source_Model.augmentation import SelectRandomFrames, VideoToTensor
    ...
    if mode == "RNN":
        ...
        for _ in trange(num_epochs):
            ...
            for x in kp_dataloader:
                ...
                loss = sum([torch.abs(gt[k][:, init_frames:] - prediction[k]).mean() for k in x ])
                print(loss)
                loss.backward()
                optimizer.step()
                optimizer.zero_grad()
