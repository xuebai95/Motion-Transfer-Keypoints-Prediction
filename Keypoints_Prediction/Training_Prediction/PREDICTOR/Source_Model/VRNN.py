...
def get_config():
  ...
  # the following three lines were adding, not initial code  
  #cfg.dim_value = 2
  #cfg.dim_jacobian = 4
  #cfg.dim_both = 6
...
def build_vrnn(cfg):
  ...
  #num_timesteps = cfg.observed_steps # the original code is last line

  input_keypoints_stack = tf.keras.Input(
      (num_timesteps, cfg.num_keypoints, 6), name='vrnn_input')
  ...
  #output_keypoints_list = [None] * cfg.observed_steps # the original code is last line
  ...
  for t in range(cfg.observed_steps):
    output_keypoints_list[t], rnn_state, kl_div_list[t] = _vrnn_iteration(
        cfg, input_keypoints_list[t], rnn_state, rnn_cell, prior_net, decoder,
         posterior_net) #scheduled_sampler_obs,
...
def build_vrnn_12(cfg):
  ...
  input_keypoints_stack = tf.keras.Input(
      (num_timesteps, cfg.num_keypoints, 6), name='vrnn_input') # this was the initial code
...
def _vrnn_iteration(cfg,
  ...
  observed_keypoints_flat = layers.Reshape([shape[0] * shape[1]])(
      input_keypoints)
  #print("observed_keypoints_flat is: " + str(observed_keypoints_flat))
  ...
  if posterior_net:
    ...
    #print("std shape is " + str(std_prior.shape))
    #print("mean shape is " + str(mean_prior.shape))
  ...
  rnn_input = layers.Concatenate(axis=-1)([observed_keypoints_flat, latent_belief]) # keypoints_for_rnn
  _, rnn_state = rnn_cell(rnn_input,[rnn_state])
  rnn_state = rnn_state[0]  # rnn_cell needs state to be wrapped in list.
  #print("output_kp shape is " + str(output_keypoints.shape))
...
def build_decoder(cfg):
  ...
  keypoints = layers.Dense(cfg.num_keypoints * 6, activation=tf.nn.tanh)(
      hidden)   # this was the initial code
...
def build_posterior_net(cfg):
  ...
  """
  rnn_state = tf.keras.Input(shape=[cfg.num_rnn_units], name='rnn_state')
  keypoints = tf.keras.Input(shape=[cfg.num_keypoints * 6], name='keypoints') # this was the initial code
